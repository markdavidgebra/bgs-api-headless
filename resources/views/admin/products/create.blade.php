...
@php
  $draftName = old('name', 'New product');
  $planInitial = strtoupper(\Illuminate\Support\Str::substr($draftName, 0, 1));
  $defaultAssuranceLines = [
      'Curated for post-treatment compatibility',
      'Authentic clinic-sourced inventory',
  ];
  $assuranceForForm = old('showcase_assurance_lines', $defaultAssuranceLines);
    ...
    <div class="container-xl">
      <div class="row align-items-center g-3">
        ...
        <div class="col">
          ...
          <h2 class="page-title mb-0">{{ $draftName ?: 'New product' }}</h2>
          <div class="text-secondary small mt-1">Add a product on one page — details, pricing, stock, and supplier.</div>
        </div>
        <div class="col-auto ms-auto d-print-none">
          <div class="btn-list">
            <a href="{{ route('admin.products') }}" class="btn">Cancel</a>
            <button type="submit" form="product-create-form" class="btn btn-primary">Save product</button>
          </div>
        </div>
      </div>
    </div>
              ...
              <div class="card-body">
                ...
                <div class="row g-3">
                  <div class="col-md-6">
                    ...
                    <input id="name" name="name" type="text" class="form-control @error('name') is-invalid @enderror"
                      value="{{ old('name') }}" required placeholder="e.g. Hydrating Serum">
                    @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                  </div>
                  <div class="col-md-6">
                    <label class="form-label" for="slug">Slug</label>
                    <input id="slug" name="slug" type="text" class="form-control @error('slug') is-invalid @enderror"
                      value="{{ old('slug') }}" placeholder="Auto from name if empty">
                  ...
                  <div class="col-12">
                    ...
                    <textarea id="description" name="description" rows="4"
                      ...
                      placeholder="Short description for staff and listings.">{{ old('description') }}</textarea>
                    @error('description') <div class="invalid-feedback">{{ $message }}</div> @enderror
                  </div>
                  <div class="col-12">
                    <label class="form-label">Storefront assurance bullets</label>
                    <div class="text-secondary small mb-2">Shown on the public product page under the description.</div>
                  ...
                  <div class="col-md-4">
                    ...
                    <div class="input-group">
                      ...
                      <input id="cost_price" name="cost_price" type="number" min="0" step="0.01" required
                        class="form-control @error('cost_price') is-invalid @enderror" value="{{ old('cost_price') }}">
                    </div>
                    ...
                    <div id="discount-fixed-wrap" class="discount-mode-panel @if(old('discount_mode', 'fixed') === 'percentage') d-none @endif">
                      <div class="input-group">
                        ...
                        <input id="discount_price" name="discount_price" type="number" min="0" step="0.01"
                          ...
                          value="{{ old('discount_price') }}" placeholder="Sale price">
                      </div>
                ...
                <div class="row g-3">
                  <div class="col-md-4">
                    ...
                    <small class="form-hint">Creates an opening “stock in” movement when greater than zero.</small>
                  </div>
                  <div class="col-md-4">
                    ...
                    <input id="minimum_stock_alert" name="minimum_stock_alert" type="number" min="0" step="1"
                      ...
                      value="{{ old('minimum_stock_alert', 5) }}">
                    ...
                    <div class="mb-2">
                      <label class="form-check">
                        ...
                        <input class="form-check-input" type="checkbox" name="is_available_for_sale" value="1"
                          @checked((string) old('is_available_for_sale', '1') === '1')>
                ...
                <div class="row g-3">
                  ...
                  <div class="col-md-4">
                    ...
                    <input id="supplier" name="supplier" type="text" class="form-control @error('supplier') is-invalid @enderror"
                      value="{{ old('supplier') }}" list="product-supplier-options" placeholder="Supplier name">
          ...
          <div class="card mb-3">
            ...
            <div class="card-body">
              <ul class="text-secondary mb-0 ps-3">
                <li>Required: name, category, cost &amp; selling price, opening stock.</li>
                <li>Set minimum alert to flag low stock on the index.</li>
                <li>Optional SKU must stay unique if provided.</li>
              </ul>
            </div>
          </div>
          <div class="card">
            ...
            <div class="card-body">
              <div class="datagrid mb-0">
                <div class="datagrid-item">
                  <div class="datagrid-title">Catalog status</div>
                  <div class="datagrid-content">{{ ucfirst(old('status', 'active')) }}</div>
                </div>
                <div class="datagrid-item">
                  <div class="datagrid-title">Stock</div>
                  <div class="datagrid-content">{{ old('stock_quantity', 0) }} {{ old('unit', 'pcs') ?: 'pcs' }}</div>
                </div>
                <div class="datagrid-item">
                  <div class="datagrid-title">Sale availability</div>
                  <div class="datagrid-content">{{ (string) old('is_available_for_sale', '1') === '1' ? 'Enabled' : 'Disabled' }}</div>
                </div>
              </div>
            </div>
          </div>
