...
@php
  ...
  $oldMembershipPlanIds = collect(old('membership_plan_ids', $affiliateCode->membershipPlans->pluck('id')))->map(fn ($v) => (string) $v)->all();
        ...
        <div class="row g-3">
          <div class="col-lg-8">
            ...
            <div class="card mb-3">
              ...
              <div class="card-body">
                <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
                  <p class="text-secondary small mb-0">Check each service this affiliate code applies to.</p>
                  @if (!$services->isEmpty())
                    <label class="form-check mb-0">
                      <input type="checkbox" class="form-check-input js-affiliate-select-all" id="aff-select-all-services"
                        data-target="affiliate-service-item">
                ...
                @else
                  <div class="border rounded p-3 bg-secondary-lt row row-cols-1 row-cols-md-2 g-2" role="group"
                    ...
                    @foreach ($services as $svc)
                      <div class="col">
                        <label class="form-check mb-0">
                          <input type="checkbox" class="form-check-input affiliate-service-item" name="service_ids[]" value="{{ $svc->id }}"
            ...
            <div class="card mb-3">
              ...
              <div class="card-body">
                <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
                  <p class="text-secondary small mb-0">Check each package this affiliate code applies to.</p>
                  @if (!$treatmentPackages->isEmpty())
                    <label class="form-check mb-0">
                      <input type="checkbox" class="form-check-input js-affiliate-select-all" id="aff-select-all-packages"
                        data-target="affiliate-package-item">
                      <span class="form-check-label">Select all</span>
                    </label>
                  @endif
                </div>
                ...
                @else
                  <div class="border rounded p-3 bg-secondary-lt row row-cols-1 row-cols-md-2 g-2" role="group"
                    ...
                    @foreach ($treatmentPackages as $pkg)
                      <div class="col">
                        <label class="form-check mb-0">
                          <input type="checkbox" class="form-check-input affiliate-package-item" name="treatment_package_ids[]"
            ...
            <div class="card mb-3">
              ...
              <div class="card-body">
                <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
                  <p class="text-secondary small mb-0">Check each product this affiliate code applies to.</p>
                  @if (!$products->isEmpty())
                    <label class="form-check mb-0">
                      <input type="checkbox" class="form-check-input js-affiliate-select-all" id="aff-select-all-products"
                        data-target="affiliate-product-item">
                      <span class="form-check-label">Select all</span>
                    </label>
                  @endif
                </div>
                ...
                @else
                  <div class="border rounded p-3 bg-secondary-lt row row-cols-1 row-cols-md-2 g-2" role="group"
                    ...
                    @foreach ($products as $prod)
                      <div class="col">
                        <label class="form-check mb-0">
                          <input type="checkbox" class="form-check-input affiliate-product-item" name="product_ids[]" value="{{ $prod->id }}"
  ...
  <script>
    ...
    (function () {
      function syncSelectAll(selectAllEl) {
        const targetClass = selectAllEl.dataset.target;
        if (!targetClass) return;
        const items = document.querySelectorAll('.' + targetClass);
        if (!items.length) return;
        const checked = [...items].filter((el) => el.checked).length;
        selectAllEl.checked = checked === items.length;
        selectAllEl.indeterminate = checked > 0 && checked < items.length;
      }

      document.querySelectorAll('.js-affiliate-select-all').forEach((selectAllEl) => {
        const targetClass = selectAllEl.dataset.target;
        if (!targetClass) return;
        const items = document.querySelectorAll('.' + targetClass);
        ...
        selectAllEl.addEventListener('change', () => {
          items.forEach((item) => {
            item.checked = selectAllEl.checked;
          });
          selectAllEl.indeterminate = false;
        });

        items.forEach((item) => {
          item.addEventListener('change', () => syncSelectAll(selectAllEl));
        });

        syncSelectAll(selectAllEl);
      });
    })();
  </script>
@endpush
