...
@section('content')
    ...
    <div class="container-xl">
      <div class="row g-2 align-items-center">
        <div class="col">
          ...
          <h2 class="page-title">Edit clinical role</h2>
          <div class="text-secondary small mt-1">Update portal permissions for this clinical role.</div>
        </div>
        <div class="col-auto ms-auto d-print-none">
          <div class="btn-list">
            <a href="{{ route('admin.clinical-staff-roles.index') }}" class="btn">Cancel</a>
            <button type="submit" form="doctor-role-edit-form" class="btn btn-primary">Save changes</button>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="page-body">
    <div class="container-xl">
      <form id="doctor-role-edit-form" method="POST" action="{{ route('admin.clinical-staff-roles.update', $role->id) }}">
        @csrf
        @method('PUT')
        <div class="card">
          <div class="card-body">
            <div class="row g-3">
              <div class="col-md-6">
                ...
                <input id="name" type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                  value="{{ old('name', $role->name) }}" placeholder="e.g. Front desk clinician" required>
                @error('name')
              ...
              <div class="col-md-6">
                ...
                <input id="role_value" type="text" name="role_value" class="form-control @error('role_value') is-invalid @enderror"
                  value="{{ old('role_value', $role->role_value) }}" placeholder="e.g. front desk clinician">
                <small class="form-hint">If empty, it will be generated from the role name.</small>
                @error('role_value')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              <div class="col-12">
                ...
                <textarea id="description" name="description" rows="3" class="form-control @error('description') is-invalid @enderror"
                  placeholder="Optional notes">{{ old('description', $role->description) }}</textarea>
                @error('description')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              <div class="col-12">
                <label class="form-label">Clinical staff portal permissions</label>
                @error('permissions')
